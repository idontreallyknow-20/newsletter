...
Commit these to the **`queue`** branch, not `main`. Writing to `main` every night would fire a
production redeploy and put a deploy on the critical path of the morning send. `vercel.json`
sets `deploymentEnabled` to `false` for the `queue` branch, so commits here build nothing at
all, not even a preview. The 5 AM job checks `queue` first and falls back to `main`, so a
hand-committed file on either branch works.
